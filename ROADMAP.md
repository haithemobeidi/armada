...
| B1 | Baseline instrumentation | ⬅ CURRENT — in progress since 2026-09-07: logger v2 built; two play runs recorded (Eco 30 min, Balanced 16 min, same game); remaining: idle run + pitch-vs-PWM sweep | A logger that records temp / PWM / CPU+GPU freq / battery power every few seconds to a CSV, pulled to the PC; one real 30-min play session and one idle session recorded |
