...
- **Fan PWM electrical facts (read 2026-09-07, `/sys/kernel/debug/pwm`):** PMIC PWM channel 3 (`c400000.spmi:pmic@1:pwm`), DT `pwm-fan` node: period 40 000 ns requested → **39 375 ns actual = 25.4 kHz carrier, above hearing** — the chopping frequency is not the audible tone. Duty at PWM 64 = 10 000 ns (25 %). The fan has its own 5 V rail (`vdd-fan-5v0-regulator`, GPIO-enabled). The DT also carries kernel `cooling-levels` `0 30 45 60 70 90 120 150`, unused while `armada-powerd` writes `pwm1` directly. The carrier is a DT/kernel value (the channel is claimed by `pwm-fan`, not exportable from sysfs) — out of `/etc` scope.
...
- **`device-overlay/` version applied:** **v2** — pushed 2026-09-11 14:23: `etc/udev/rules.d/99-armada-hide-internal-ufs.rules` (v1, re-sent byte-identical, `HintIgnore: true` since 2026-09-06) + `etc/systemd/system-sleep/20-odin3-wifi-off-in-sleep` (mode 0755, `bash -n` clean; `rfkill block wifi` on suspend, `unblock` on wake — sleep 0.75 W → 0.47 W on this unit, ROADMAP B4). Revert: delete the hook file; `rfkill unblock wifi` if a wake ever leaves Wi-Fi off.
- **Fan curve (UI-owned file, set 2026-09-11 by direct edit on the user's ask, same path + `armada-power reload` the Fans tab uses):** `relaxed = 0:0,50:0,51:51,65:51,76:77,82:102,88:153,98:255`, `[fan] min_pwm = 0` — fan off below 50 °C. Backups on the device: `/var/tmp/power-profiles.conf.bak-20260911` (before) and `-2` (the 45 °C draft). Recorded in `device-state/`.
- **UI-owned state as last pulled:** `device-state/` (2026-09-11 12:14, after the fan-curve edit; before that 2026-09-06 14:48, after the internal install — byte-identical to the 2026-09-05 pull). Balanced has `gpu_max = 0.80` (set by the user in the Power tab; seemed to reduce the whine a bit — **but it is a no-op: 0.80 × the 1100 MHz table top = 880 → clamps to 832 = uncapped; see `docs/ARMADA_CONTROL.md` → GPU Max, L-16**. **The fresh internal deployment reset it to factory**; the user set it again by hand at 14:24 on 2026-09-06, before the re-pull — so UI tweaks do not survive an installer run), everything else factory; `abl.conf` `auto_update_enabled=1`; `game-tweaks.json` and `input-calibration.json` do not exist yet (nothing set).

- **Outside `/etc`, harmless, not an overlay:** `/var/tmp/armada-baseline/` holds the B1 logger (`baseline-logger.sh`, = repo `tools/baseline-logger.sh` v2), the one-off `wait-balanced.sh`, and the two run CSVs (also pulled to `device-data/`). Nothing running there after 2026-09-07 03:23. Also `/var/tmp/sleep-test-*/` (2026-09-11 sleep cycles: logs, interrupt snapshots, the sleep-debug report) and the one-off scripts `/var/tmp/{sleep-probe,cpu30,idle20,irq20,masters,votes,gpio104*,bisect,sleep-test*,sleep10*,sleep-deep*,hookcheck,set-relaxed}.sh`. Cleanup if ever wanted: `rm -rf /var/tmp/armada-baseline /var/tmp/sleep-test-* /var/tmp/*.sh`.
