...
- [ ] L-10 (2026-09-06) User: in Steam (Gaming Mode → Settings → Storage) confirm the reformatted SD card appears and pick where new installs go (internal `sda20` is the fast one; the card is bulk space).
- [ ] L-11 (2026-09-06) `docs/DEVICE.md` "UI-owned state as last pulled" is from the SD-card install; the internal deployment started from image defaults (`gpu_max` 0.80 etc. are gone). Re-pull `device-state/` at the next pause and have the user redo the Power-tab tweaks they want.
