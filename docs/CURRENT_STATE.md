...
**Rebase `odin3-tuning` onto `upstream/main` (L-9, D-2)** — 14+ commits behind, and the device now runs `20260906.41d2e10`, newer than our fork point. Then **open B1 — Baseline instrumentation:** declare pause-points, write the on-device CSV logger, record one idle and one play session, run the pitch-vs-PWM sweep with the user listening. Re-pull `device-state/` first (L-11: the internal deployment reset the UI tweaks).
...
Open: L-3 (user: AYN warranty check), L-4 (user: off-PC copy of the ABL backup — the device-side copy is gone), L-5 (download-plateau experiment, now on internal storage), L-6 (game froze on a Performance switch mid shader-compile; B3 repro), L-10 (user: check the SD card shows in Steam's Storage settings), L-11 (re-pull `device-state/`, redo Power-tab tweaks). See `docs/SESSION_LEDGER.md`.
...
None. Device/repo parity: nothing in `device-overlay/` yet (v0). **`device-state/` is stale** — it describes the SD-card install's `/etc`; the internal deployment started from image defaults (L-11).
...
- **Fresh deployments lose `/etc` and `/var`:** SSH off, key gone, UI tweaks gone. Expect the same after any future `armada-installer` run or B10 reflash — not after ordinary OTAs (three-way `/etc` merge, D-4).
