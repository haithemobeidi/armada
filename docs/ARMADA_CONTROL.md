...
| GPU Max (%) | `gpu_max` | 35–100 | Ceiling — **but the ratio is taken against the top of the devfreq table (1100 MHz), not the 832 MHz kernel cap** (`armada-powerd`: `choose_at_most(freqs, int(freqs[-1] * gpu_max))`, `freqs` = `available_frequencies`). So 0.80 → 880 → **832 MHz = no cap at all**: Eco's factory 0.80 and the user's Balanced 0.80 both leave the GPU uncapped (measured 2026-09-07: Eco sat at 832 MHz for 78 % of a 30-min session). Real ceilings: anything ≥ 0.76 → 832 (none); **0.60 → 660 MHz**; 0.55 → 607; 0.48 → 525. `gpu_min` uses the same basis (Performance's 1.0 → 1100 → clamps to 832, still pinned at max). A **frame-rate cap** or lower **game resolution** stays the preferred lever. B9 candidate: ratio against the usable max, or show the resulting MHz in the UI (L-16). |
