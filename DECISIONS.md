...
**Same day:** the three template fixes were upstreamed to the Knowledge Base and this project resynced. `protected_branches = ["main"]` and `upstream_ref = "upstream/main"` are set in `protocol.json`, so the `main` guard, the upstream drift count at start, and `upstream +N` in the statusline are hook-enforced again. Only the firmware/binary suffix skip stays gone (the `/end` backstop ignores blobs by hand).
